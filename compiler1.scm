...
(define-simple-class Goto (Emitter)
	...
	((emit compiler ::Compiler classes ::Classes c ::ClassType mi ::MethodInfo code ::CodeAttr needed ::Type) ::Type
		...
		(if (and output (code:reachableHere)) (code:emitGoto label))
		Type:voidType
	)
)
...
(define-simple-class Emitters (Emitter)
	...
	((emit compiler ::Compiler classes ::Classes c ::ClassType mi ::MethodInfo code ::CodeAttr needed ::Type) ::Type
		...
		(for-each
			(lambda (emitter)
				(if (not (eq? emitter #!null))
					(set! type (emitter:emit compiler classes c mi code needed))
				)
			)
			emitters
		)
...
(define-simple-class AndOrEmitter (Emitter)
	...
	(default ::Emitter)
	(inv ::boolean)
	((*init* tok ::Token i ::int tfTok ::Emitter skipTok ::Emitter default ::Emitter inv ::boolean)
		(invoke-special Emitter (this) '*init*)
		(set! (this):tok tok)
		(set! (this):i i)
		(set! (this):tfTok tfTok)
		(set! (this):skipTok skipTok)
		(set! (this):default default)
		(set! (this):inv inv)
	)
	((emit compiler ::Compiler classes ::Classes c ::ClassType mi ::MethodInfo code ::CodeAttr needed ::Type) ::Type
		(if (= (+ i 1) (tok:ops:size))
			(compiler:emitIf classes inv tok i (Emitters (Emitter[] default skipTok)) #!null c mi code needed)
			(compiler:emitIf classes inv tok i (AndOrEmitter tok (+ i 1) tfTok skipTok default inv) tfTok c mi code needed)
		)
	)
)
...
(define-simple-class Compiler ()
			...
			(begin
				...
				(if (first:val:equals "define-class")
					(begin
						...
						(do ((i 3 (+ i 1)))
							((= i (length tok:ops)))
							(compile_class_mods (tok:ops:get i) c)
						)
						(set! tok:val c)
						#!null
					)
	...
	((compile_class_mods tok ::Token c ::ClassType)
		(if (tok:what:equals "V")
			(begin
				(define nmod (accessModifiers:get tok:val))
				(*:addModifiers c nmod)
			)
		)
	)
				...
				(if (first:what:equals "(") ;method definition
					(begin
						...
						(do ()
							...
							(if (and (mod:what:equals "V") (accessModifiers:containsKey mod:val))
								(begin
									(set! mods (logior mods (accessModifiers:get mod:val)))
									(set! i (+ i 1))
								)
								(set! end #t)
							)
					...
					(begin ;field definition
						...
						(do ((i 2 (+ i 1)))
							...
							(if (and (mod:what:equals "V") (accessModifiers:containsKey mod:val))
								(begin
									(define nmod (accessModifiers:get mod:val))
	...
	((emitIf_ classes ::Classes inv tok ::Token i compare true ::Emitter false ::Emitter c ::ClassType mi ::MethodInfo code ::CodeAttr needed ::Type) ::Type
		...
		(if (or (and (not inv) (compare:equals "&&")) (and inv (compare:equals "||")))
			(begin
				...
				((AndOrEmitter tok i falseTok (Goto skipL) true inv):emit (this) classes c mi code needed)
				...
				(true:emit (this) classes c mi #!null needed)
			)
		(if (or (and (not inv) (compare:equals "||")) (and inv (compare:equals "&&")))
			(begin
				(define skipL ::Label (Label))
				...
				((AndOrEmitter tok i trueTok (Goto skipL) false (not inv)):emit (this) classes c mi code needed)
			...
			(begin
				(define falseLabel (instance? false Goto))
				...
				(if (compare1Ops:containsKey compare)
					(begin
						...
						(if output (code:emitGotoIfCompare1 label (invertComp inv (compare1Ops:get compare))))
					)
					(begin
						(compile_ classes (tok:ops:get i) c mi code unknownType)
						(compile_ classes (tok:ops:get (+ i 1)) c mi code unknownType)
						(if output (code:emitGotoIfCompare2 label (invertComp inv (compare2Ops:get compare))))
					)
				)
				;(code:emitIf)
				(define type ::Type (true:emit (this) classes c mi code needed))
