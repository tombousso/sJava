			...
			(if (= t:prec p)
				(begin
					...
					(if (w:equals "{")
						(begin
							(set! e (- e (block i e "{" "}")))
							(set! (toks (- i 1)) (Token t:line "generic" "" -1 (java.util.ArrayList (toks:get (- i 1)) (toks:get i))))
							(shiftDown i 1)
							(set! e (- e 1))
							(set! i (- i 1))
						)
...
(define-simple-class Emitter ()
	((emit compiler ::Compiler classes ::Classes c ::ClassType m ::Method code ::CodeAttr scopes ::java.util.ArrayDeque needed ::Type labels ::java.util.HashMap) ::Type Type:voidType)
)
...
(define-simple-class TokensEmitter (Emitter)
	...
	((emit compiler ::Compiler classes ::Classes c ::ClassType m ::Method code ::CodeAttr scopes ::java.util.ArrayDeque needed ::Type labels ::java.util.HashMap) ::Type
		(define type ::Type Type:voidType)
		(do ((j i (+ j 1)))
			((= j l))
			(set! type (compiler:compile_ classes (toks:get j) c m code scopes needed labels))
		)
		type
	)
)
...
(define-simple-class TokenEmitter (Emitter)
	...
	((emit compiler ::Compiler classes ::Classes c ::ClassType m ::Method code ::CodeAttr scopes ::java.util.ArrayDeque needed ::Type labels ::java.util.HashMap) ::Type
		(compiler:compile_ classes tok c m code scopes needed labels)
	)
)
...
(define-simple-class Goto (Emitter)
	...
	((emit compiler ::Compiler classes ::Classes c ::ClassType m ::Method code ::CodeAttr scopes ::java.util.ArrayDeque needed ::Type labels ::java.util.HashMap) ::Type (if (not (eq? code #!null)) (code:emitGoto label)) Type:voidType)
)
...
(define-simple-class Emitters (Emitter)
	...
	((emit compiler ::Compiler classes ::Classes c ::ClassType m ::Method code ::CodeAttr scopes ::java.util.ArrayDeque needed ::Type labels ::java.util.HashMap) ::Type
		(define type ::Type  Type:voidType)
		(for-each
			(lambda (emitter) (set! type (emitter:emit compiler classes c m code scopes needed labels)))
			emitters
		)
		type
	)
	...
	((get tok ::Token) ::Type
		(if (tok:what:equals "generic")
			(begin
				...
				(do ((i 0 (+ i 1)))
					((= i (params:ops:size)))
					(set! (tparams i) ((this):get (as String (as Token (params:ops:get i)):val)))
				)
...
(define-simple-class Compiler ()
	...
	((compile_al classes ::Classes toks ::java.util.ArrayList i e c ::ClassType m ::Method code ::CodeAttr scopes ::java.util.ArrayDeque needed labels ::java.util.HashMap) ::Type[]
		(define l (toks:size))
		(define types (Type[] length: (- e i)))
		(do ((j i (+ j 1)))
			((>= j e))
			(set! (types (- j i)) (compile_ classes (toks:get j) c m code scopes (if (eq? needed #!null) #!null (if (instance? needed Type[]) (needed (- j i)) needed)) labels))
		)
		types
	)
	...
	((getVar scopes ::java.util.ArrayDeque name) ::AVar
		;(println "SEARCH" name)
			...
			(begin ;method definition
				...
				(if (not (mi:method:isAbstract))
					(begin
						...
						(define scopes ::java.util.ArrayDeque (java.util.ArrayDeque))
						(code:pushScope)
						(scopes:push mi:scope)
						(compile_al classes tok:ops 2 (- (length tok:ops) 1) c mi:method #!null scopes Type:voidType (java.util.HashMap))
						(compile_ classes (tok:ops:get (- (length tok:ops) 1)) c mi:method #!null scopes (mi:method:getReturnType) (java.util.HashMap))
						(compile_al classes tok:ops 2 (- (length tok:ops) 1) c mi:method code scopes Type:voidType (java.util.HashMap))
						(compile_ classes (tok:ops:get (- (length tok:ops) 1)) c mi:method code scopes (mi:method:getReturnType) (java.util.HashMap))
						(code:popScope)
						(code:emitReturn)
					)
				)
	...
	;tok = condition
	((emitIf_ classes ::Classes inv tok ::Token i compare true ::Emitter false ::Emitter c ::ClassType m ::Method code ::CodeAttr scopes ::java.util.ArrayDeque needed ::Type labels ::java.util.HashMap) ::Type
		(define output (not (eq? code #!null)))
		(if (compare:equals "!")
			(begin
				(emitIf classes (not inv) tok i true false c m code scopes needed labels)
			)
		(if (or (and (not inv) (compare:equals "&&")) (and inv (compare:equals "||")))
			(begin
				...
				(do ((i 1 (+ i 1)))
					((= i (- (tok:ops:size) 1)))
					(emitIf classes (not inv) tok i falseTok #!null c m code scopes needed labels)
				)
				(emitIf classes (not inv) tok (- (tok:ops:size) 1) falseTok #!null c m code scopes needed labels)
				(if output (code:pushScope))
				(scopes:push (java.util.HashMap))
				(define type (true:emit (this) classes c m code scopes needed labels))
				...
				(if (not (eq? false #!null))
					(begin
						(if output (code:pushScope))
						(scopes:push (java.util.HashMap))
						(false:emit (this) classes c m code scopes needed labels)
						(if output (code:popScope))
						(scopes:pop)
					)
				)
		...
		(if (or (and (not inv) (compare:equals "||")) (and inv (compare:equals "&&")))
			(begin
				...
				(do ((i 1 (+ i 1)))
					((= i (- (tok:ops:size) 1)))
					(emitIf classes inv tok i trueTok #!null c m code scopes needed labels)
				)
				(emitIf classes inv tok (- (tok:ops:size) 1) trueTok #!null c m code scopes needed labels)
				(if (not (eq? false #!null))
					(begin
						(if output (code:pushScope))
						(scopes:push (java.util.HashMap))
				...
				(if output (code:pushScope))
				(scopes:push (java.util.HashMap))
				(define type ::Type (true:emit (this) classes c m code scopes needed labels))
				(if output (code:popScope))
				(scopes:pop)
				(if output (skipL:define code))
				type
			...
			(begin
				...
				(if (compare1Ops:containsKey compare)
					(begin
						(compile_ classes (tok:ops:get i) c m code scopes unknownType labels)
						(if output (code:emitGotoIfCompare1 skip (invertComp inv (compare1Ops:get compare))))
					)
					(begin
						(compile_ classes (tok:ops:get i) c m code scopes unknownType labels)
						(compile_ classes (tok:ops:get (+ i 1)) c m code scopes unknownType labels)
	...
	((emitIf classes ::Classes inv tok ::Token i true ::Emitter false ::Emitter c ::ClassType m ::Method code ::CodeAttr scopes ::java.util.ArrayDeque needed ::Type labels ::java.util.HashMap) ::Type
		(define cond ::Token (tok:ops:get i))
		(if (and (cond:what:equals "(") (isCompare (as Token (cond:ops:get 0)):val))
			(emitIf_ classes inv cond 1 (as Token (cond:ops:get 0)):val true false c m code scopes needed labels)
			(emitIf_ classes inv tok i "!=0" true false c m code scopes needed labels)
		)
	)
	((emitInvoke name type ::Type classes ::Classes tok ::Token c ::ClassType m ::Method code ::CodeAttr scopes ::java.util.ArrayDeque needed ::Type labels ::java.util.HashMap) ::Type
		(define output (not (eq? code #!null)))
		(define types (compile_al classes tok:ops 1 (length tok:ops) c m #!null scopes unknownType labels))
		;(println tok type name types output)
		...
		(compile_al classes tok:ops 1 (length tok:ops) c m code scopes params labels)
	...
	((compile_ classes ::Classes tok ::Token c ::ClassType m ::Method code ::CodeAttr scopes ::java.util.ArrayDeque needed ::Type labels ::java.util.HashMap) ::Type
		...
		;(println "COMPILING" tok);code output tok needed)
		(define result ::Type
			...
			(if (tok:what:equals ":")
				(begin
					(define first ::Token (tok:ops:get 0))
					(define class ::ClassType (compile_ classes first c m code scopes unknownType labels))
			...
			(if (tok:what:equals "V")
				...
				(if (tok:val:equals "null")
					...
					(begin
						...
						(if (eq? class #!null)
							(begin
								(define found ::AVar (getVar scopes tok:val))
				...
				(begin
					...
					(if (first:val:equals "begin")
						(begin
							(if output (code:pushScope))
							(scopes:push (java.util.HashMap))
							(compile_al classes tok:ops 1 (- (length tok:ops) 1) c m code scopes Type:voidType labels)
							(define type ::Type (compile_ classes (tok:ops:get (- (length tok:ops) 1)) c m code scopes needed labels))
							(if output (code:popScope))
							(scopes:pop)
							type
						)
					(if (first:val:equals "label")
						(begin
							(define name ::String (as Token (tok:ops:get 1)):val)
					...
					(if (first:val:equals "goto")
						(begin
							(define name ::String (as Token (tok:ops:get 1)):val)
							(if (labels:containsKey name)
								(if output (code:emitGoto (labels:get name)))
								(begin
									(define label ::Label (Label))
							...
							(define type ::Type
								(if (= (length tok:ops) 4)
									(begin
										...
										(compile_ classes (tok:ops:get 3) c m code scopes type labels)
										type
									)
									(compile_ classes (tok:ops:get 2) c m code scopes unknownType labels)
								)
							)
							(define name (as Token (tok:ops:get 1)):val)
					...
					(if (first:val:equals "try")
						(begin
							...
							(define type ::Type (compile_ classes (tok:ops:get 1) c m code scopes needed labels))
							(if output (code:emitCatchStart Type:javalangThrowableType))
							(if output (code:emitPop 1))
							(compile_ classes (tok:ops:get 2) c m code scopes needed labels)
					...
					(if (first:val:equals "instance?")
						(begin
							(compile_ classes (tok:ops:get 1) c m code scopes unknownType labels)
						...
						(begin
							...
							(if (out:what:equals ":")
								(begin
									...
									(compile_ classes (tok:ops:get 2) c m code scopes (field:getType) labels)
									(if (field:getStaticFlag)
								...
								(begin
									(define var ::AVar (getVar scopes out:val))
									(compile_ classes (tok:ops:get 2) c m code scopes var:type labels)
					...
					(if (first:val:equals "aset")
						(begin
							(define type ::ArrayType (compile_ classes (tok:ops:get 1) c m code scopes unknownType labels))
							(compile_ classes (tok:ops:get 2) c m code scopes unknownType labels)
							(compile_ classes (tok:ops:get 3) c m code scopes type:elements labels)
					...
					(if (first:val:equals "aget")
						(begin
							(define type ::ArrayType (compile_ classes (tok:ops:get 1) c m code scopes unknownType labels))
							(compile_ classes (tok:ops:get 2) c m code scopes Type:intType labels)
					...
					(if (first:val:equals "as")
						(begin
							...
							(define out ::Type (compile_ classes (tok:ops:get 2) c m code scopes (cast:getRawType) labels))
					...
					(if (first:val:equals "if")
						(begin
							(emitIf classes #f tok 1 (TokenEmitter (tok:ops:get 2)) (if (= (tok:ops:size) 4) (TokenEmitter (tok:ops:get 3)) #!null) c m code scopes needed labels)
						)
					(if (first:val:equals "while")
						(begin
							(define start ::Label (Label))
							(if output (start:define code))
							(emitIf classes #f tok 1 (Emitters (Emitter[] (TokensEmitter tok:ops 2 (length tok:ops)) (Goto start))) #!null c m code scopes needed labels)
						)
					(if (isCompare first:val)
						(begin
							(emitIf_ classes #f tok 1 first:val (TokenEmitter (Token tok:line "V" "true")) (TokenEmitter (Token tok:line "V" "false")) c m code scopes Type:booleanType labels)
						)
					(if (binOps:containsKey first:val)
						(begin
							(define types ::java.util.List (java.util.Arrays:asList (compile_al classes tok:ops 1 (tok:ops:size) c m #!null scopes unknownType labels)))
							...
							(compile_ classes (tok:ops:get 1) c m code scopes otype labels)
							(do ((i 2 (+ i 1)))
								((= i (tok:ops:size)))
								(compile_ classes (tok:ops:get i) c m code scopes otype labels)
						...
						(begin
							...
							(if (first:what:equals ":")
								(begin ;method call
									...
									(emitInvoke name (compile_ classes (first:ops:get 0) c m code scopes unknownType labels) classes tok c m code scopes unknownType labels)
								)
								(begin ;constructor
									...
									(if (instance? type ArrayType)
										(begin
											...
											(define i0
												(if (and (not (eq? len #!null)) (len:what:equals ":") ((as Token (len:ops:get 0)):val:equals "len"))
													(begin (compile_ classes (len:ops:get 1) c m code scopes unknownType labels) 2)
										...
										(begin
											...
											(if output (code:emitDup))
											(emitInvoke "<init>" type classes tok c m code scopes unknownType labels)
											type
										)
									)
								)
